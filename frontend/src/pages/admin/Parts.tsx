  ...
  return (
    <Layout pageTitle="Parts Master" pageSubtitle="Manage part numbers and their associated gauge requirements" pageIcon={Package}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total Parts', value: parts.length, accent: 'from-indigo-500 to-purple-500', cardBg: 'bg-gradient-to-br from-indigo-100 to-indigo-50/60', cardBorder: 'border-indigo-200' },
          { label: 'Active', value: parts.filter((p) => p.isActive).length, accent: 'from-emerald-500 to-teal-500', cardBg: 'bg-gradient-to-br from-emerald-100 to-emerald-50/60', cardBorder: 'border-emerald-200' },
          { label: 'Inactive', value: parts.filter((p) => !p.isActive).length, accent: 'from-red-500 to-rose-500', cardBg: 'bg-gradient-to-br from-red-100 to-red-50/60', cardBorder: 'border-red-200' },
          { label: 'Characteristics', value: new Set(parts.map((p) => p.characteristic).filter(Boolean)).size, accent: 'from-blue-500 to-cyan-500', cardBg: 'bg-gradient-to-br from-blue-100 to-blue-50/60', cardBorder: 'border-blue-200' },
        ].map((s) => (
