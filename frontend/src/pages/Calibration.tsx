...
// ═══════════════════════════════════════════════════════════════════
// PER-GAUGE PENDING CALIBRATION TYPE (localStorage helpers)
// ═══════════════════════════════════════════════════════════════════
...
export default function Calibration() {
  ...
  const sendToCalibration = () => {
    ...
    gaugeStorage.update(selectedGaugeForCal, {
    ...
    <Layout pageTitle="Calibration" pageSubtitle="Record and track internal and external gauge calibrations" pageIcon={ClipboardCheck}>
      ...
      <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {/* ─── NEW: Pending notification button ───────────────── */}
          <button
            ...
            title={
              pendingCount > 0
                ? `${pendingCount} gauge(s) awaiting result entry`
                : 'No pending calibrations'
            }
          >
            <div className="relative">
      ...
      {/* ═══════════════════════════════════════════════════════════ */}
      {/* NEW: Pending Calibrations Drawer (slides in from right)     */}
      {/* ═══════════════════════════════════════════════════════════ */}
      {pendingDrawerOpen && (
        <>
          {/* Backdrop */}
              ...
              {pendingCount === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    No gauges are currently under calibration.
                  </p>
                  <button
                    onClick={() => {
                ...
                <div className="space-y-3">
                  {underCalibrationGauges.map((g) => {
                    ...
                    return (
                      <div
                        key={g.id}
                        className="bg-white border border-gray-200 hover:border-blue-300 rounded-xl p-4 transition hover:shadow-md"
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-gray-500 mt-0.5 truncate">
                              {g.type} · {g.department} · {g.location}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          {pendingType === 'Internal' ? (
                            <button
                              onClick={() => {
                                setPendingDrawerOpen(false);
                                setIntForm({ ...emptyInternalForm, gaugeId: g.id });
                                setIntModalOpen(true);
                              }}
                            ...
                            <button
                              onClick={() => {
                                setPendingDrawerOpen(false);
                                setExtForm({ ...emptyExternalForm, gaugeId: g.id });
                                setExtModalOpen(true);
                              }}
            ...
            {pendingCount > 0 && (
              <div className="border-t border-gray-100 p-4 flex-shrink-0 bg-gray-50">
                <p className="text-xs text-gray-500 text-center">
                  Click "Record Result" to finish the calibration and update the gauge status.
                </p>
              </div>
            )}
          </div>
      ...
      <Modal
        ...
        footer={
          <>
            <button
              onClick={() => { setSendToCalOpen(false); setSelectedGaugeForCal(''); }}
              className="px-4 py-2.5 border border-gray-300 text-gray-700 hover:bg-gray-50 font-semibold rounded-xl transition text-sm"
            >
              Cancel
            </button>
      ...
      >
        <div className="space-y-4">
          <div className="p-3 bg-blue-50 border border-blue-200 text-blue-800 rounded-lg text-sm flex items-start gap-2">
            ...
            <span>
              This will mark the gauge as <strong>Under Calibration</strong> (blue on Heat Map). After you record the result, its status will update to <strong>Available</strong> (Pass) or <strong>Under Review</strong> (Fail).
            </span>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1.5">
              After clicking "Send to Calibration", the {calType.toLowerCase()} result-entry form will open automatically.
            </p>
          </div>

          <div>
            <label className="block text-xs font-semibold text-gray-800 mb-1.5">
              Select Available Gauge <span className="text-red-500">*</span>
            </label>
            <select
      ...
      >
        ...
        <div className="space-y-4">
          <div>
            ...
            {intForm.gaugeId && allGauges.find((g) => g.id === intForm.gaugeId)?.status === 'Under Calibration' && (
              <p className="text-xs text-blue-600 mt-1 flex items-center gap-1">
                <Clock className="w-3 h-3" strokeWidth={2} />
                This gauge is currently under calibration.
              </p>
            )}
          </div>
          <div>
      ...
      >
        ...
        <div className="space-y-4">
          <div>
            ...
            {extForm.gaugeId && allGauges.find((g) => g.id === extForm.gaugeId)?.status === 'Under Calibration' && (
              <p className="text-xs text-blue-600 mt-1 flex items-center gap-1">
                <Clock className="w-3 h-3" strokeWidth={2} />
                This gauge is currently under calibration.
              </p>
