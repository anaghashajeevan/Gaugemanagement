...
export default function CAPAPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list.sort((a, b) => {
      // Open first, then by target date
  ...
  const getGaugeStatus = (gaugeId: string) => gauges.find((x) => x.id === gaugeId)?.status || '';

  // Get linked source (MSA study or Calibration record) for context
  const getLinkedSource = (capa: CAPAType) => {
    if (capa.sourceType === 'MSA') {
      return msaStudies.find((m) => m.id === capa.sourceId);
    } else if (capa.sourceType === 'Calibration') {
      return calibrations.find((c) => c.id === capa.sourceId);
    }
    return null;
  };
  ...
  const handleCloseCapa = () => {
    ...
    let newGaugeStatus: string;
    switch (closeAction) {
      case 'available': newGaugeStatus = 'Available'; break;
      case 'scrapped': newGaugeStatus = 'Scrapped'; break;
      case 'recalibrate': newGaugeStatus = 'Under Calibration'; break;
      default: newGaugeStatus = 'Available';
    }
  ...
  const detailCapa = capas.find((c) => c.id === detailId);
  const linkedSource = detailCapa ? getLinkedSource(detailCapa) : null;

  const openCount = capas.filter((c) => c.status === 'Open').length;
  const overdueCount = capas.filter((c) => c.status === 'Open' && new Date(c.targetDate) < new Date()).length;
  const quarantinedGauges = gauges.filter((g) => g.status === 'Under Review').length;
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex items-center gap-1.5">
          {row.sourceType === 'MSA' ? (
            <BarChart3 className="w-3.5 h-3.5 text-purple-500" strokeWidth={2} />
          ) : (
            <ClipboardCheck className="w-3.5 h-3.5 text-indigo-500" strokeWidth={2} />
          )}
          <StatusBadge status={row.sourceType} />
        </div>
      ),
    },
    { header: 'Gauge Status', cell: (row) => <StatusBadge status={getGaugeStatus(row.gaugeId)} /> },
    {
      header: 'Root Cause',
      cell: (row) => <span className="truncate max-w-[180px] block text-sm">{row.rootCause}</span>,
    },
    { header: 'Responsible', cell: (row) => <span>{row.responsiblePerson}</span> },
    {
      header: 'Target Date',
      cell: (row) => {
        const overdue = row.status === 'Open' && new Date(row.targetDate) < new Date();
  ...
  return (
     <Layout pageTitle="CAPA" pageSubtitle="Track corrective and preventive actions for calibration and MSA failures" pageIcon={AlertTriangle}>
      {quarantinedGauges > 0 && (
        <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-red-600 mt-0.5">
              These gauges cannot be issued until their CAPA is closed.
            </p>
          </div>
        </div>
      )}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}
        ...
        <div className="space-y-4">
          {!editingId && (
            <div className="p-3 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg text-sm flex items-start gap-2">
              <AlertTriangle className="w-4 h-4 flex-shrink-0 mt-0.5" strokeWidth={2} />
              Creating a CAPA will <strong>quarantine</strong> the selected gauge. It cannot be issued until this CAPA is closed.
            </div>
          )}
          <div className="grid grid-cols-2 gap-4">
            <div>
      ...
      {/* Detail Modal — with linked MSA/Calibration */}
      <Modal open={!!detailCapa} onClose={() => setDetailId(null)}
        title="CAPA Details" subtitle={detailCapa ? getGaugeLabel(detailCapa.gaugeId) : ''} maxWidth="2xl"
        footer={<>
          <button onClick={() => setDetailId(null)} className="px-4 py-2.5 border border-gray-300 text-gray-700 hover:bg-gray-50 font-semibold rounded-xl transition text-sm">Close</button>
          {detailCapa?.status === 'Open' && (
            <button onClick={() => { setDetailId(null); openCloseModal(detailCapa.id); }}
          ...
          <div className="space-y-4">
            ...
            {detailCapa.status === 'Open' && getGaugeStatus(detailCapa.gaugeId) === 'Under Review' && (
              <div className="p-3 bg-red-50 border border-red-200 text-red-700 rounded-lg text-sm flex items-start gap-2">
                <Gauge className="w-4 h-4 flex-shrink-0 mt-0.5" strokeWidth={2} />
                <div>
                  <strong>Gauge is quarantined.</strong> It cannot be issued until this CAPA is closed and gauge status is updated.
                </div>
              </div>
            )}

            {/* ─── LINKED SOURCE — MSA STUDY DETAILS ─────────────── */}
            {detailCapa.sourceType === 'MSA' && linkedSource && (() => {
              const study = linkedSource as any; // MSAStudy
              return (
                <div className="bg-purple-50 border border-purple-100 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-purple-700 uppercase tracking-wider">
                        Linked MSA Study Result
                      </span>
                    </div>
                    <button
                      onClick={() => { setDetailId(null); navigate('/msa'); }}
                      className="text-xs text-purple-600 hover:text-purple-700 font-semibold flex items-center gap-1"
                    >
                      View in MSA <ExternalLink className="w-3 h-3" strokeWidth={2} />
                    </button>
                  </div>

                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    <div className="bg-white rounded-lg p-3">
                      <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Study Type</p>
                      <p className="text-sm font-bold text-gray-800 mt-1">{study.studyType}</p>
                    </div>
                    <div className="bg-white rounded-lg p-3">
                      <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Result</p>
                      <p className="text-sm font-bold text-red-600 mt-1 font-mono">
                        {study.studyType === 'GRR' && `${study.grrPercent?.toFixed(1)}%`}
                        {study.studyType === 'Bias' && study.biasValue?.toFixed(4)}
                        {study.studyType === 'Linearity' && `Max: ${study.linearityMaxBias?.toFixed(4)}`}
                        {study.studyType === 'Uncertainty' && `±${study.expandedUncertainty?.toFixed(4)}`}
                      </p>
                    </div>
                    <div className="bg-white rounded-lg p-3">
                      <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Date</p>
                      <p className="text-sm font-bold text-gray-800 mt-1">{study.completedDate || study.createdDate}</p>
                    </div>
                    ...
                    {study.studyType === 'GRR' && (
                      <>
                        <div className="bg-white rounded-lg p-3">
                          <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">%EV</p>
                          <p className="text-sm font-bold text-gray-800 mt-1 font-mono">{study.evPercent?.toFixed(2)}%</p>
                        </div>
                        <div className="bg-white rounded-lg p-3">
                          <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">%AV</p>
                          <p className="text-sm font-bold text-gray-800 mt-1 font-mono">{study.avPercent?.toFixed(2)}%</p>
                        </div>
                        <div className="bg-white rounded-lg p-3">
                          <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">NDC</p>
                          <p className={`text-sm font-bold mt-1 font-mono ${(study.ndc || 0) >= 5 ? 'text-emerald-700' : 'text-red-700'}`}>
                            {study.ndc}
                          </p>
                        </div>
                      </>
                    ...
                    {study.studyType === 'Bias' && (
                      <>
                        <div className="bg-white rounded-lg p-3">
                          <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Reference</p>
                          <p className="text-sm font-bold text-gray-800 mt-1 font-mono">{study.biasReferenceValue?.toFixed(4)}</p>
                        </div>
                        <div className="bg-white rounded-lg p-3">
                          <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">t-Statistic</p>
                          <p className="text-sm font-bold text-gray-800 mt-1 font-mono">{study.biasTStatistic?.toFixed(3)}</p>
                        </div>
                        <div className="bg-white rounded-lg p-3">
                          <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Significant?</p>
                          <p className={`text-sm font-bold mt-1 ${study.biasSignificant ? 'text-red-700' : 'text-emerald-700'}`}>
                            {study.biasSignificant ? 'Yes' : 'No'}
                          </p>
                        </div>
                      </>
                    )}
                    ...
                    {study.studyType === 'Linearity' && (
                      <>
                        <div className="bg-white rounded-lg p-3">
                          <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Slope</p>
                          <p className="text-sm font-bold text-gray-800 mt-1 font-mono">{study.linearitySlope?.toFixed(4)}</p>
                        </div>
                        <div className="bg-white rounded-lg p-3">
                          <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">R²</p>
                          <p className="text-sm font-bold text-gray-800 mt-1 font-mono">{study.linearityRSquared?.toFixed(3)}</p>
                        </div>
                      </>
                    )}
                  </div>
                  ...
                  <p className="text-xs text-purple-700 mt-3 flex items-center gap-1">
                    ...
                    Operators involved: {study.operatorNames?.join(', ')}
                  </p>
                </div>
              );
            })()}
            ...
            {/* ─── LINKED CALIBRATION ─────────────────────────────── */}
            {detailCapa.sourceType === 'Calibration' && linkedSource && (() => {
              const cal = linkedSource as any;
              return (
                <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <ClipboardCheck className="w-4 h-4 text-indigo-600" strokeWidth={2} />
                    <span className="text-xs font-bold text-indigo-700 uppercase tracking-wider">
                      Linked Calibration Record
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-white rounded-lg p-3">
                      <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Type</p>
                      <p className="text-sm font-bold text-gray-800 mt-1">{cal.type}</p>
                    </div>
                    <div className="bg-white rounded-lg p-3">
                      <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Result</p>
                      <p className="text-sm font-bold text-red-600 mt-1">{cal.result}</p>
                    </div>
                    <div className="bg-white rounded-lg p-3">
                      <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Date</p>
                      <p className="text-sm font-bold text-gray-800 mt-1">{cal.date}</p>
                    </div>
                  </div>
                </div>
              );
      ...
      <Modal open={closeModalOpen} onClose={() => setCloseModalOpen(false)}
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="space-y-2">
              {CLOSE_ACTIONS.map((action) => (
                <button key={action.value} onClick={() => setCloseAction(action.value)}
                  className={`w-full text-left p-4 rounded-xl border transition ${
                    closeAction === action.value ? 'border-indigo-300 bg-indigo-50 ring-2 ring-indigo-200' : 'border-gray-200 hover:border-indigo-200'
                  }`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                      closeAction === action.value ? 'border-indigo-500' : 'border-gray-300'
                    }`}>
                      {closeAction === action.value && <div className="w-2 h-2 rounded-full bg-indigo-500" />}
                    </div>
                    <div>
                      <p className={`text-sm font-semibold ${closeAction === action.value ? 'text-indigo-700' : 'text-gray-700'}`}>{action.label}</p>
                      <p className="text-xs text-gray-400 mt-0.5">{action.description}</p>
                    </div>
                  </div>
                </button>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-800 mb-1.5">Effectiveness Verification</label>
